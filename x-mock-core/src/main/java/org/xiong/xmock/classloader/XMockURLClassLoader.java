...
import org.xiong.xmock.classloader.jar.Handler;
...
public class XMockURLClassLoader extends URLClassLoader {
	...
	/**
	 * Create a new {@link XMockURLClassLoader} instance.
	 * @param urls the URLs from which to load classes and resources
	 * @param parent the parent class loader for delegation
	 */
	public XMockURLClassLoader(URL[] urls, ClassLoader parent) {
	 ...
	 * @param parent the parent class loader for delegation
	 */
	public XMockURLClassLoader(boolean exploded, URL[] urls, ClassLoader parent) {
	 ...
	 * @since 2.3.1
	 */
	public XMockURLClassLoader(boolean exploded, Archive rootArchive, URL[] urls, ClassLoader parent) {
		super(urls, parent);
...
//		}
		...
		try {
			...
			URL xmockUrl = super.findResource(path);
			if(xmockUrl == null){
				return super.loadClass(name, resolve);
			}else{
				return this.xmockLoadClass(name, resolve);
			}
		}
		finally {
			Handler.setUseFastConnectionExceptions(false);
	...
	protected Class<?> xmockLoadClass(String name, boolean resolve) throws ClassNotFoundException
	{
	...
	private void clearCache(URLConnection connection) throws IOException {
		...
		if (jarFile instanceof org.xiong.xmock.classloader.jar.JarFile) {
			((org.xiong.xmock.classloader.jar.JarFile) jarFile).clearCache();
		}
	}
